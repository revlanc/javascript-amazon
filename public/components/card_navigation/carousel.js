...
export default class Carousel extends Subscriber {
  ...
  checkMovable(idx) {
    if (idx == 0) this.prevBtn.classList.add('arrow-disable');
  ...
  setTransform(idx) {
    ...
    this.camera.style.cssText = `transform: translateX(${-WIDTH_IN_PERCENT *
      idx}%); transition: transform ${this.option.duration}ms`;
  }
